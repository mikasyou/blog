class RGB {
  ...
  constructor() {
    ...
    this.r = Math.min(Math.ceil(63 + weight * 20), 255)
    this.g = Math.min(Math.ceil(114 + weight * 20), 255)
    this.b = Math.min(Math.ceil(175 + weight * 20), 255)
  }
  ToString(): string {
    ...
    const g = Math.ceil(this.g + persent * 30)
    const r = Math.ceil(this.r + persent * 30)
...
export class GlassEffectBoard {
  ...
  private calcRandom(seed: number): number {
  ...
  constructor(param: { id: string; area: { col: number; row: number } }) {
    ...
    // 平局每列分部点，每行分布点
    const columnWidth = this.width / (this.area.col - 1)
    const rowHeight = this.height / (this.area.row - 1)
    ...
    for (let col = 0; col < this.area.col - 1; col++) {
      for (let row = 0; row < this.area.row - 1; row++) {
        ...
        this.triangles.push(
          {
            color: new RGB(),
            points: [points[col][row], points[col + 1][row], points[col][row + 1]]
          },
          {
            color: new RGB(),
            points: [points[col][row + 1], points[col + 1][row], points[col + 1][row + 1]]
          }
        )
      }
    }
