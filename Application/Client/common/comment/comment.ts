...
import { pageService, apiService } from "../api";
...
export class CommentComponent {
  ...
  constructor() {
    ...
    this.btnReply.addEventListener("click", (e) => { this.postComment(e.target as HTMLButtonElement); });
  }
  ...
  onReplyClick(e: MouseEvent) {
    ...
    if (!replyBtn.classList.contains("active")) {
      ...
      this.replyType = 1;
      let comment = e.composedPath().find((m: HTMLElement) => m.classList && (m.classList.contains("comment-post") || m.classList.contains("comment-reply"))) as HTMLDivElement;
      comment.after(this.commentMainForm);

      this.reply = {
        floorid: parseInt(replyBtn.dataset.floorid),
        replyid: parseInt(replyBtn.dataset.replyid),
  ...
  postComment(btn: HTMLButtonElement) {
    ...
    if (this.replyType === 0) {
      apiService.postComment({
        ...
        articleid: parseInt(btn.dataset.article)
      })
        .then(json => {
